...
# Set default values for optional feature flags to prevent "UNSET" warnings
export GPS_TRACKING="${GPS_TRACKING:-false}"
export CELLULAR_TRACKING="${CELLULAR_TRACKING:-false}"
export ENHANCED_FAILOVER="${ENHANCED_FAILOVER:-false}"

# Set default logging configuration
export LOG_TAG="${LOG_TAG:-StarlinkMonitor}"
export LOG_DIR="${LOG_DIR:-/etc/starlink-logs}"
export STATE_DIR="${STATE_DIR:-/tmp/run}"
...
if [ "${DEBUG:-0}" = "1" ]; then
    ...
    log_debug "  GPS_TRACKING: ${GPS_TRACKING:-UNSET}"
    log_debug "  CELLULAR_TRACKING: ${CELLULAR_TRACKING:-UNSET}"
    log_debug "  ENHANCED_FAILOVER: ${ENHANCED_FAILOVER:-UNSET}"
...
for possible_path in \
    ...
    if [ -f "$possible_path" ]; then
        # shellcheck source=/dev/null
        . "$possible_path"
        log_debug "Placeholder utilities loaded from: $possible_path"
...
# Set up centralized logging for all monitoring activities
UNIFIED_LOG_FILE="${LOG_DIR}/starlink_monitor.log"

# Function to log to both standard output and unified log file
unified_log() {
    local level="$1"
    local message="$2"
...
# Override log functions to include unified logging
...
original_log_warning() { log_warning "$@"; }
log_warning() {
    original_log_warning "$@"
    unified_log "WARNING" "$*"
}
...
log_success() {
    original_log_success "$@"
    unified_log "SUCCESS" "$*"
}

# Set default values for variables that may not be in config (non-duplicated ones)
...
# Enhanced feature flags (already set in configuration mapping, but ensure they exist)
MULTI_SOURCE_GPS="${MULTI_SOURCE_GPS:-false}"
ENHANCED_FAILOVER="${ENHANCED_FAILOVER:-false}"
...
# Debug configuration
log_debug "GPS_TRACKING=$GPS_TRACKING, CELLULAR_TRACKING=$CELLULAR_TRACKING"
log_debug "ENHANCED_FAILOVER=$ENHANCED_FAILOVER, MULTI_SOURCE_GPS=$MULTI_SOURCE_GPS"
...
collect_gps_data() {
    ...
    if [ "$MULTI_SOURCE_GPS" = "true" ] && command -v gpsctl >/dev/null 2>&1; then
    ...
    if [ -z "$lat" ] && [ "$CELLULAR_TRACKING" = "true" ]; then
...
collect_cellular_data() {
    ...
    if [ "$CELLULAR_TRACKING" != "true" ]; then
...
analyze_connection_quality() {
    ...
    if [ "$ENHANCED_FAILOVER" = "true" ]; then
        ...
        elif [ "$quality_factors" -eq 1 ] && [ "$CELLULAR_TRACKING" = "true" ]; then
...
trigger_failover() {
    ...
    # Get current metric using configured MWAN_MEMBER
...
restore_starlink() {
    ...
    # Reset to default metric using configured MWAN_MEMBER
    if safe_execute "uci set mwan3.${MWAN_MEMBER}.metric=${METRIC_GOOD:-10}" "Reset mwan3 metric to default"; then
        if safe_execute "uci commit mwan3" "Commit mwan3 changes"; then
            if safe_execute "/etc/init.d/mwan3 reload" "Reload mwan3 service"; then
