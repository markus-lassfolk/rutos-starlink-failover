...
setup_cron_jobs() {
    ...
    (
        ...
        echo "* * * * * $SCRIPTS_DIR/starlink_monitor-rutos.sh"
    ) | crontab -
    ...
    if [ "$ENABLE_STARLINK_MONITORING" = "true" ]; then
        (
            crontab -l 2>/dev/null
            echo "* * * * * $SCRIPTS_DIR/starlink_logger-rutos.sh"
        ) | crontab -
...
# === SCRIPT GENERATORS ===
create_starlink_monitor_script() {
    cat >"$SCRIPTS_DIR/starlink_monitor-rutos.sh" <<'EOF'
#!/bin/sh
...
EOF
    ...
    chmod +x "$SCRIPTS_DIR/starlink_monitor-rutos.sh"
    log_success "Starlink monitor script created"
}

create_starlink_logger_script() {
    cat >"$SCRIPTS_DIR/starlink_logger-rutos.sh" <<'EOF'
#!/bin/sh
...
EOF
    ...
    chmod +x "$SCRIPTS_DIR/starlink_logger-rutos.sh"
...
test_scripts() {
    ...
    scripts=(
        "/root/starlink_monitor-rutos.sh"
        "/root/starlink_logger-rutos.sh"
...
test_cron_jobs() {
    ...
    if echo "$cron_jobs" | grep -q "starlink_monitor-rutos.sh"; then
        log_pass "Quality monitoring scheduled"
    else
        log_fail "Quality monitoring not scheduled"
    fi
    
    if echo "$cron_jobs" | grep -q "starlink_logger-rutos.sh"; then
