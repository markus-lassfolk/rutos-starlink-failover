...
setup_cron_jobs() {
    ...
    (
        ...
        echo "# Starlink monitoring - using unified scripts (recommended)"
        echo "*/5 * * * * $SCRIPTS_DIR/starlink_monitor_unified-rutos.sh"
    ) | crontab -
    ...
    if [ "$ENABLE_STARLINK_MONITORING" = "true" ]; then
        (
            crontab -l 2>/dev/null
            echo "# Starlink logging - using unified scripts (recommended)"
            echo "*/1 * * * * $SCRIPTS_DIR/starlink_logger_unified-rutos.sh"
        ) | crontab -
...
# === SCRIPT GENERATORS ===
create_starlink_monitor_script() {
    # Create unified monitoring script (recommended)
    cat >"$SCRIPTS_DIR/starlink_monitor_unified-rutos.sh" <<'EOF'
#!/bin/sh
...
main() {
    ...
    if [ "${DEBUG:-0}" = "1" ] || [ "${VERBOSE:-0}" = "1" ]; then
        printf "[DEBUG] %s v%s\n" "deploy-starlink-solution-rutos.sh" "$SCRIPT_VERSION" >&2
    fi
    log_debug "==================== SCRIPT START ==================="
    log_debug "Script: deploy-starlink-solution-rutos.sh v$SCRIPT_VERSION"
...
EOF
    ...
    chmod +x "$SCRIPTS_DIR/starlink_monitor_unified-rutos.sh"
    log_success "Unified Starlink monitor script created"
}

create_starlink_logger_script() {
    cat >"$SCRIPTS_DIR/starlink_logger_unified-rutos.sh" <<'EOF'
#!/bin/sh
...
EOF
    ...
    chmod +x "$SCRIPTS_DIR/starlink_logger_unified-rutos.sh"
    log_success "Unified Starlink logger script created"
}
...
create_pushover_notifier_script() {
    cat >"$HOTPLUG_DIR/99-pushover_notify-rutos.sh" <<EOF
#!/bin/sh
...
test_scripts() {
    ...
    scripts=(
        "/root/starlink_monitor_unified-rutos.sh"
        "/root/starlink_logger_unified-rutos.sh"
    ...
    if [ -x "/etc/hotplug.d/iface/99-pushover_notify-rutos.sh" ]; then
...
test_cron_jobs() {
    ...
    if echo "$cron_jobs" | grep -q "starlink_monitor_unified-rutos.sh"; then
        log_pass "Quality monitoring scheduled"
    else
        log_fail "Quality monitoring not scheduled"
    fi
    
    if echo "$cron_jobs" | grep -q "starlink_logger_unified-rutos.sh"; then
...
finalize_deployment() {
    ...
    chmod 755 "$HOTPLUG_DIR"/99-pushover_notify-rutos.sh 2>/dev/null || true
