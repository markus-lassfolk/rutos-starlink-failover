...
The **Intelligent Starlink Monitoring System v3.0** now uses a **persistent storage architecture** designed specifically for RUTOS firmware upgrade survival:

### **Persistent Directory Structure:**
```
/opt/starlink/                          # Main installation (PERSISTENT)
...
```
/root/starlink_monitor_unified-rutos.sh -> /opt/starlink/bin/starlink_monitor_unified-rutos.sh
/root/config.sh -> /opt/starlink/config/config.sh
```
...
# 2. Recreate symlinks
ln -sf /opt/starlink/bin/starlink_monitor_unified-rutos.sh /root/starlink_monitor_unified-rutos.sh
ln -sf /opt/starlink/config/config.sh /root/config.sh

# 3. Restore daemon service
cp /opt/starlink/templates/starlink-monitor.init /etc/init.d/starlink-monitor
...
```
/opt/starlink/                              # SURVIVES firmware upgrade
...
mkdir -p /opt/starlink/{bin,config,logs,state,lib,templates}

# 2. Move existing files
mv /root/starlink_monitor_unified-rutos.sh /opt/starlink/bin/
mv /root/config.sh /opt/starlink/config/
...
sed -i 's|LOG_DIR="/root/logs"|LOG_DIR="/opt/starlink/logs"|g' /opt/starlink/config/config.sh
```

## 🛡️ Firmware Upgrade Best Practices

### **Before Firmware Upgrade:**
1. **Verify persistent storage**: `ls -la /opt/starlink/`
2. **Backup configuration**: `cp /opt/starlink/config/config.sh /opt/starlink/backup-$(date +%Y%m%d)/`
3. **Note current status**: `/opt/starlink/bin/starlink_monitor_unified-rutos.sh status`

### **After Firmware Upgrade:**
1. **Run recovery script**: `/opt/starlink/bin/recover-after-firmware-upgrade.sh`
2. **Verify functionality**: `/opt/starlink/bin/starlink_monitor_unified-rutos.sh status`
3. **Check logs**: `tail -f /opt/starlink/logs/rutos-lib.log`
...
/opt/starlink/bin/starlink_monitor_unified-rutos.sh validate
/opt/starlink/bin/starlink_monitor_unified-rutos.sh discover
```
...
ls -la /opt/starlink/
ls -la /opt/starlink/bin/
ls -la /opt/starlink/config/
...
/opt/starlink/bin/starlink_monitor_unified-rutos.sh status

# Check daemon service
/etc/init.d/starlink-monitor status

# Verify monitoring functionality
tail -20 /opt/starlink/logs/rutos-lib.log
```
...
1. **Always use persistent storage** (`/opt/starlink/`) for the intelligent monitoring system
...
This persistent storage architecture ensures that your **Intelligent Starlink Monitoring System** becomes a **permanent, resilient part of your RUTOS infrastructure** that survives firmware upgrades and continues providing intelligent network management without interruption! 🛡️
