...
export NOTIFY_ON_CRITICAL=1 # Always notify on critical errors (recommended: 1)
#
# SOFT_FAIL: Starlink degraded but still usable (high latency, packet loss)
export NOTIFY_ON_SOFT_FAIL=1 # Notify on soft failover events (recommended: 1)
#
# HARD_FAIL: Starlink completely down, switched to cellular backup
export NOTIFY_ON_HARD_FAIL=1 # Notify on hard failover events (recommended: 1)
#
# RECOVERY: Starlink recovered, switched back from cellular
export NOTIFY_ON_RECOVERY=1 # Notify when system recovers/failback (recommended: 1)
#
# INFO: Status updates, monitoring health, debug information
export NOTIFY_ON_INFO=0 # Notify on info/status (0=quiet, 1=verbose)
...
export METRIC_GOOD=1 # Metric when Starlink is working well
export METRIC_BAD=10 # Metric when Starlink is degraded (forces cellular usage)

# --- File Paths ---

# System directories for persistent storage
# NOTE: /var/log is wiped on reboot in OpenWrt/RUTOS - use /overlay/ for persistence
export STATE_DIR="/tmp/run"              # Runtime state files (temporary)
export LOG_DIR="/overlay/starlink-logs"  # Log files directory (persistent across reboots)
export DATA_DIR="/overlay/starlink-data" # Data storage directory (persistent across reboots)

# --- Binary Paths ---

# Installed binary locations (set by install script)
export GRPCURL_CMD="/root/grpcurl" # gRPC client for Starlink API
export JQ_CMD="/root/jq"           # JSON processor

# --- RUTOS API Configuration (for GPS) ---

# RUTX50 router management interface
# Default: 192.168.80.1 (standard RUTX50 LAN IP)
export RUTOS_IP="192.168.80.1"

# RUTX50 login credentials
# Set these to your router's admin credentials
export RUTOS_USERNAME="YOUR_RUTOS_USERNAME"
export RUTOS_PASSWORD="YOUR_RUTOS_PASSWORD"

# --- Logging Configuration ---

# System logging settings
export LOG_TAG="StarlinkSystem" # Syslog tag for filtering logs
export LOG_RETENTION_DAYS=7     # How long to keep log files

# --- Advanced Settings ---

# Timeout values in seconds
export API_TIMEOUT=10             # Starlink API request timeout
export HTTP_TIMEOUT=15            # HTTP request timeout (for RUTOS API)
export GPS_ACCURACY_THRESHOLD=100 # GPS accuracy threshold in meters
export MOVEMENT_THRESHOLD=500     # Movement detection threshold in meters
