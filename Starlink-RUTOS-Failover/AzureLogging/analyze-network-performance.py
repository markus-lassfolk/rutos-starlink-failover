...
class NetworkAnalyzer:
    ...
    def download_data(self, days_back: int = 30) -> None:
        ...
        try:
            ...
            performance_container = self.blob_client.get_container_client("starlink-performance")
            self._download_logs(performance_container, start_date, end_date, "performance")
            
            logger.info(f"Downloaded {len(self.system_logs)} system log entries")
            logger.info(f"Downloaded {len(self.performance_data)} performance data points")
            
        except Exception as e:
            logger.error(f"Error downloading data: {e}")
            raise
    
    def _download_logs(self, container_client, start_date: datetime, end_date: datetime, log_type: str) -> None:
        """
    ...
    def _parse_log_line(self, line: str, log_date: datetime.date) -> Optional[Dict]:
        ...
        if match:
            ...
            try:
                timestamp = datetime.strptime(f"{log_date.year} {timestamp_str}", '%Y %b %d %H:%M:%S')
    ...
    def _identify_events(self, log_entry: Dict) -> None:
        ...
        # Failover events
        failover_keywords = [
            'failover', 'switching to backup', 'starlink down', 'primary wan down',
            'backup wan up', 'connection restored', 'wan failover'
        ]
        ...
        # Reboot events
        reboot_keywords = [
            'system startup', 'kernel:', 'init:', 'booting', 'system halt',
                ...
                if len(parts) >= 8:  # Minimum expected columns
                    try:
                        ...
                        performance_entry = {
                            ...
                            'downlink_throughput_bps': float(parts[2]) if parts[2] else 0,
                            ...
                            'obstruction_duration_s': float(parts[6]) if parts[6] else 0,
                        ...
                        if len(parts) > 8:
                            performance_entry.update({
                                'currently_obstructed': parts[8].lower() == 'true' if parts[8] else False,
                                'snr': float(parts[9]) if len(parts) > 9 and parts[9] else 0,
                                'alerts_thermal_throttle': parts[10].lower() == 'true' if len(parts) > 10 and parts[10] else False,
                                'alerts_thermal_shutdown': parts[11].lower() == 'true' if len(parts) > 11 and parts[11] else False,
                                'dishy_state': parts[13] if len(parts) > 13 else '',
                                'mobility_class': parts[14] if len(parts) > 14 else '',
                            })
                        
                        # Add GPS data if available (new columns)
                        if len(parts) > 18:
                            performance_entry.update({
                                'latitude': float(parts[18]) if parts[18] and parts[18] != '' else None,
                                'longitude': float(parts[19]) if parts[19] and parts[19] != '' else None,
                                'altitude_m': float(parts[20]) if len(parts) > 20 and parts[20] and parts[20] != '' else None,
                                'speed_kmh': float(parts[21]) if len(parts) > 21 and parts[21] and parts[21] != '' else None,
                                'heading_deg': float(parts[22]) if len(parts) > 22 and parts[22] and parts[22] != '' else None,
                                'gps_source': parts[23] if len(parts) > 23 else 'none',
                                'gps_satellites': int(parts[24]) if len(parts) > 24 and parts[24] and parts[24] != '' else 0,
                                'gps_accuracy_m': float(parts[25]) if len(parts) > 25 and parts[25] and parts[25] != '' else None,
                            })
                        ...
                        logger.warning(f"Could not parse performance line: {line[:100]}... Error: {e}")
    ...
    def _check_performance_thresholds(self, entry: Dict) -> None:
        ...
        if entry['downlink_throughput_bps'] < 10_000_000:  # 10 Mbps
            violations.append(f"Low downlink: {entry['downlink_throughput_bps']/1_000_000:.1f} Mbps")
        
        # Obstruction issues
        if entry['obstruction_fraction'] > 0.02:  # 2%
            violations.append(f"High obstruction: {entry['obstruction_fraction']*100:.1f}%")
    ...
    def analyze_failover_patterns(self) -> Dict:
        ...
        peak_hour = Counter(failover_hours).most_common(1)[0] if failover_hours else (0, 0)
    ...
    def analyze_gps_patterns(self) -> Dict:
        ...
        """
        gps_data = [p for p in self.performance_data if p.get('latitude') is not None and p.get('longitude') is not None]
        ...
        # GPS source analysis
        gps_sources = df['gps_source'].value_counts().to_dict() if 'gps_source' in df.columns else {}
    ...
    def analyze_mobility_patterns(self) -> Dict:
        ...
        """
        mobile_data = [p for p in self.performance_data if p.get('speed_kmh') is not None]
    ...
    def _analyze_movement_patterns(self, df: pd.DataFrame) -> Dict:
        ...
        avg_distance_per_measurement = sum(distances) / len(distances) if distances else 0
        
        return {
            'total_distance_km': round(total_distance, 2),
            'avg_distance_per_measurement_m': round(avg_distance_per_measurement * 1000, 2),
            'max_distance_between_points_m': round(max(distances) * 1000, 2) if distances else 0
        }
    ...
    def _analyze_location_performance(self, df: pd.DataFrame) -> Dict:
        ...
        if len(location_performance) > 0:
            ...
            best_throughput_location = location_performance['downlink_throughput_bps'].idxmax()
            worst_throughput_location = location_performance['downlink_throughput_bps'].idxmin()
            ...
            return {
                ...
                'best_latency_location': {
                    ...
                    'avg_latency_ms': location_performance.loc[best_latency_location, 'ping_latency_ms']
                },
                'worst_latency_location': {
                    'lat': worst_latency_location[0],
                    'lon': worst_latency_location[1],
                    'avg_latency_ms': location_performance.loc[worst_latency_location, 'ping_latency_ms']
                },
                'best_throughput_location': {
                    'lat': best_throughput_location[0],
                    'lon': best_throughput_location[1],
                    'avg_throughput_mbps': location_performance.loc[best_throughput_location, 'downlink_throughput_bps'] / 1_000_000
                },
                'worst_throughput_location': {
                    'lat': worst_throughput_location[0],
                    'lon': worst_throughput_location[1],
                    'avg_throughput_mbps': location_performance.loc[worst_throughput_location, 'downlink_throughput_bps'] / 1_000_000
                }
            }
    ...
    def _calculate_coverage_area(self, df: pd.DataFrame) -> float:
        ...
        lon_km = lon_range * 111 * abs(df['latitude'].mean()) / 90  # Adjust for longitude at latitude
    ...
    def _analyze_speed_performance_correlation(self, df: pd.DataFrame) -> Dict:
        ...
        mobility_performance = df.groupby('mobility_state').agg({
            'ping_latency_ms': 'mean',
            'ping_drop_rate': 'mean',
            'downlink_throughput_bps': 'mean',
        ...
        if 'speed_kmh' in df.columns and len(df) > 10:
            correlations = {
                ...
                'speed_vs_throughput': df['speed_kmh'].corr(df['downlink_throughput_bps']),
            }
    ...
    def correlate_events_and_performance(self) -> Dict:
        ...
        for reboot in self.reboot_events:
            ...
            if relevant_performance:
                avg_latency = sum(p['ping_latency_ms'] for p in relevant_performance) / len(relevant_performance)
                avg_loss = sum(p['ping_drop_rate'] for p in relevant_performance) / len(relevant_performance)
    ...
    def generate_threshold_recommendations(self) -> Dict:
        ...
        packet_loss_95th = sorted(packet_losses)[int(len(packet_losses) * 0.95)] if packet_losses else 0
        
        throughput_5th = sorted(throughputs)[int(len(throughputs) * 0.05)] if throughputs else 0
        ...
        recommendations = {
            ...
            'recommended_thresholds': {
                'latency_warning_ms': int(latency_95th * 1.1),  # 10% above 95th percentile
                'latency_critical_ms': int(latency_99th * 1.1), # 10% above 99th percentile
                'packet_loss_warning_pct': round(packet_loss_95th * 100 * 1.2, 2), # 20% above 95th percentile
                'min_throughput_mbps': round(throughput_5th / 1_000_000 * 0.8, 1) # 20% below 5th percentile
            },
            'violation_analysis': {
                'total_violations': len(self.threshold_violations),
                'violation_rate_pct': len(self.threshold_violations) / len(self.performance_data) * 100 if self.performance_data else 0
            }
        }
    ...
    def create_visualizations(self, output_dir: str = './analysis_output') -> None:
        ...
        try:
            ...
            gps_data = [p for p in self.performance_data if p.get('latitude') is not None]
            if gps_data:
    ...
    def _plot_performance_trends(self, output_dir: str) -> None:
        ...
        # Packet Loss
        axes[0, 1].plot(df['timestamp'], df['ping_drop_rate'] * 100, alpha=0.7, color='red')
        axes[0, 1].set_title('Packet Loss')
        axes[0, 1].set_ylabel('Packet Loss (%)')
        axes[0, 1].grid(True)
        
        # Throughput
        axes[1, 0].plot(df['timestamp'], df['downlink_throughput_bps'] / 1_000_000, alpha=0.7, label='Downlink')
        axes[1, 0].plot(df['timestamp'], df['uplink_throughput_bps'] / 1_000_000, alpha=0.7, label='Uplink')
        ...
        # Obstructions
        axes[1, 1].plot(df['timestamp'], df['obstruction_fraction'] * 100, alpha=0.7, color='orange')
    ...
    def _plot_event_timeline(self, output_dir: str) -> None:
        ...
        if self.failover_events:
            ...
            ax.scatter(failover_times, [1] * len(failover_times), 
                      c='red', s=100, alpha=0.7, label='Failover Events')
        
        # Plot reboot events
        if self.reboot_events:
            reboot_times = [event['timestamp'] for event in self.reboot_events]
            ax.scatter(reboot_times, [2] * len(reboot_times), 
                      c='orange', s=100, alpha=0.7, label='Reboot Events')
    ...
    def _plot_threshold_violations(self, output_dir: str) -> None:
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/threshold_violations.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_gps_coverage(self, output_dir: str, gps_data: List[Dict]) -> None:
        ...
        scatter = ax1.scatter(df['longitude'], df['latitude'], 
                             c=df['ping_latency_ms'], cmap='RdYlBu_r', 
                             s=20, alpha=0.7)
    ...
    def _plot_performance_by_location(self, output_dir: str, gps_data: List[Dict]) -> None:
        """
        ...
        axes[1, 0].scatter(df['latitude'], df['downlink_throughput_bps']/1_000_000, alpha=0.6, color='green')
        ...
        axes[1, 1].scatter(df['longitude'], df['downlink_throughput_bps']/1_000_000, alpha=0.6, color='green')
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/performance_by_location.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_mobility_analysis(self, output_dir: str, mobile_data: List[Dict]) -> None:
        ...
        # Speed vs Throughput
        axes[1, 0].scatter(df['speed_kmh'], df['downlink_throughput_bps']/1_000_000, alpha=0.6, color='green')
    ...
    def generate_report(self, output_file: str = 'network_analysis_report.json') -> Dict:
        """
        ...
        report = {
            ...
            'data_summary': {
                ...
                'analysis_period': {
                    'start': min(p['timestamp'] for p in self.performance_data).isoformat() if self.performance_data else None,
                    'end': max(p['timestamp'] for p in self.performance_data).isoformat() if self.performance_data else None
                }
            },
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Analyze network performance and failover patterns')
    ...
    try:
        ...
        report = analyzer.generate_report(f'{args.output_dir}/network_analysis_report.json')
        ...
        print(f"Performance Data Points: {report['data_summary']['performance_data_points']:,}")
        
        print(f"\nFailover Events: {failover_analysis.get('failover_count', 0)}")
        print(f"Average Failovers/Day: {failover_analysis.get('avg_failovers_per_day', 0):.2f}")
        
        if 'avg_latency' in performance_analysis:
            print(f"\nAverage Latency: {performance_analysis['avg_latency']:.1f} ms")
            print(f"Average Packet Loss: {performance_analysis['avg_packet_loss']:.2f}%")
            print(f"Average Downlink Speed: {performance_analysis['avg_downlink_mbps']:.1f} Mbps")
            print(f"Performance Issues: {performance_analysis.get('degradation_events', 0)}")
        ...
        if 'recommended_thresholds' in recommendations:
            ...
            print(f"  Packet Loss Warning: {rec.get('packet_loss_warning_pct', 'N/A')}%")
