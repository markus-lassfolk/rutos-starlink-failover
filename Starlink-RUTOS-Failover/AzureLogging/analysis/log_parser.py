...
class LogParser:
    ...
    def parse_system_logs(self, content: str, log_date: datetime.date) -> List[Dict[str, Any]]:
        """
    ...
    def parse_performance_data(self, content: str, log_date: datetime.date) -> List[Dict[str, Any]]:
        """
    ...
    def _parse_csv_row(self, row: Dict[str, str], log_date: datetime.date) -> Dict[str, Any]:
        """
    ...
    def _classify_event(self, process: str, message: str) -> str:
        ...
        # Failover events
        if any(keyword in message_lower for keyword in ['failover', 'switching', 'route change', 'metric']):
            return 'FAILOVER'
            
        # Reboot events
        if any(keyword in message_lower for keyword in ['reboot', 'restart', 'startup', 'shutdown']):
            return 'REBOOT'
            
        # Network events
        if any(keyword in message_lower for keyword in ['network', 'interface', 'connection', 'link']):
            return 'NETWORK'
            
        # GPS events
        if any(keyword in message_lower for keyword in ['gps', 'location', 'coordinate']):
            return 'GPS'
            
        # Starlink specific
        if any(keyword in message_lower for keyword in ['starlink', 'dishy', 'satellite']):
            return 'STARLINK'
